...
router.put("/update/:id", updateTaskPermission, async (req, res) => {
    if (!req.params.id)
        return sendError(400, "Error: Please provide id", res);
    if (!req.body)
        return sendError(400, "Body is missing", res);
    try {
        const updatedTaskMessage = await TaskModel.updateOne({ _id: req.params.id }, {
            $set: {
                ...req.body,
            },
        });
...
router.put("/:id", changeStatusPermission, async (req, res) => {
    if (!req.params.id)
        return sendError(400, "Error: Please provide id", res);
    try {
        const updatedTask = await TaskModel.updateOne({ _id: req.params.id }, {
            $set: {
